...
QQ 群：[点击加入](https://qm.qq.com/cgi-bin/qm/qr?k=cX461O2DhAyGuaaSKdh-9aVPKhW8RpKv&jump_from=webapi&qr=1)。你也可以在群里分享你自己的应用说明书和 `SKILL.md` 操作指引。

讨论区：[GitHub Discussions](https://github.com/iamr0s/ruto-phone-mcp/discussions)
...
- 增加用于手机控制、任务执行和会话查看的 Web 界面。
- 提供更便捷的配置方式，以及更清晰的配置校验。
- 增加内置的 Skill 管理能力，支持安装、更新、启用、停用与卸载。
- 支持更多形式的通知渠道，用于任务进度和完成结果推送。

## 参与开发

我平时工作比较繁忙，这个项目的推进速度未必会一直很快。如果你觉得这个项目有价值，欢迎一起参与开发、完善文档、补充 Skills，让整个系统逐步变得更完整。
...
如果你需要普通的 HTTP 传输，可使用 `http`。对于 MCP 客户端，优先使用 `streamable-http`。
